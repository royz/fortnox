...
const OUTPUT_PATH = path.join(import.meta.dirname, "../spec/openapi.json");
const HASH_FILE_PATH = path.join(import.meta.dirname, "../spec/openapi.hash");
...
async function downloadAndExtractSpec() {
  ...
  if (!response.ok) {
    throw new Error(`Failed to download ReDoc page: ${response.status} ${response.statusText}`);
  }
