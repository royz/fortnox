...
*/
export async function getSpecFromFile(type: "original" | "patched", bypassHashCheck?: boolean): Promise<OpenAPIV3.Document> {
  ...
  if (type === "original" && !bypassHashCheck) {
    ...
    const expectedHash = (await readFile(hashPath, "utf-8")).trim().split("\n")?.at(-1);
    if (fileHash !== expectedHash) {
      throw new Error(`Original spec file hash mismatch! Expected hash: ${expectedHash}, got ${fileHash}. Re-download the spec using "pnpm run download".`);
    }
  }
