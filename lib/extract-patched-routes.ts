...
export async function extractPatchedRoutes() {
  ...
  for (const [path, pathItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      const successResponse = (operation.responses?.["200"] || operation.responses?.["201"]) as
        | OpenAPIV3.ResponseObject
        | undefined;
      const successResponseSchema =
        successResponse?.content?.["application/json"]?.schema;
      const isPdfResponse =
        !successResponseSchema && !!successResponse?.content?.["application/pdf"];
      ...
      } else if (successResponseSchema && isReferenceObject(successResponseSchema)) {
        responseBodyTypeName = generateTypeNameFromRef(successResponseSchema.$ref);
