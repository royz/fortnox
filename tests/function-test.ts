...
type InitFortnoxOptions =
  ...
  | { proxyUrl: string; apiKey: string; tenantId: string; skipValidation?: boolean; };

function validateRequestOptions(reqOptions: RequestOptions): ZodError | undefined {
  const methods = (paths as any)[reqOptions.path];

  const schemas = methods[reqOptions.method.toLowerCase()]?.input as ZodType | undefined;
  ...
  try {
    ...
    if (response.status < 300) { // Success
      ...
      return {
        error: {
          ErrorSource: "fortnox",
          ErrorInformation: (resposneData as FortnoxErrorRespose).ErrorInformation,
        } satisfies ErrorRespose,
        data: null,
      }
    }
...
type FortnoxResult<TData> = Promise<{ error: ErrorRespose; data: null } | { error: null; data: TData }>;
type FortnoxMethods<TPath extends keyof Paths> = {
  [TMethod in keyof Paths[TPath]]: undefined extends InferInput<Paths[TPath][TMethod]>
  ? (options?: InferInput<Paths[TPath][TMethod]>) => FortnoxResult<InferOutput<Paths[TPath][TMethod]>>
  : (options: InferInput<Paths[TPath][TMethod]>) => FortnoxResult<InferOutput<Paths[TPath][TMethod]>>;
};

function initFortnox(initOptions: InitFortnoxOptions) {
  return function fortnox<TPath extends keyof Paths>(path: TPath): FortnoxMethods<TPath> {
